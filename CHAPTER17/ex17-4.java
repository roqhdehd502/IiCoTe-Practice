...
class Main {
  public static void main(String[] args) {
    // int n = 6; // 헛간의 개수(번호)
    // int m = 7; // 연결된 헛간들의 사건의 수

    // 두 헛간의 번호
    int[] a = {3, 4, 3, 1, 1, 2, 5}; 
    int[] b = {6, 3, 2, 3, 2, 4, 2};

    int hide = 0; // 숨어야 하는 헛간 번호(거리가 같은 헛간이 여러개면 가장 작은 헛간 번호만 출력)
    int dist = 0; // 그 헛간까지의 거리
    int cnt = 0; // 그 헛간과 같은 거리를 갖는 헛간의 개수

    /* 문제를 이해못하니... 로직을 구현할 수가 없다... */

    System.out.println(hide + " " + dist + " " + cnt); // 결과값 출력
  }
}
