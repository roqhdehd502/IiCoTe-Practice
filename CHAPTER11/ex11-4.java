...
추상화
=====
그리디 알고리즘을 이용해여 현재 상태를 1부터 target-1까지의 모든 금액을 만들 수 있는 상태를 체크한다.
예를 들어 단위가 1, 1, 2, 3, 9인 5개의 동전이 있을때... 1부터 16(1 + 1 + 2 + 3 + 9)까지의 모든 금액을 만들 수 있으므로
- 1원 : 1
- 2원 : 2 (or) 1 + 1 (개인의견: 일단 조합 가짓 수에 집중하는게 아닌 합에 집중을 해보자...)
- 3원 : 3 (or) ...
- 4원 : 1 + 1 + 2
- 5원 : 2 + 3 (or) ...
- 6원 : 1 + 2 + 3
- 7원 : 1 + 1 + 2 + 3
- 8원? : 1 + 1 + 2 + 3 + ...?
즉, 8원을 조합할 수 있는지 확인 후 만들 수 없으므로 답을 8로 도출한다.
